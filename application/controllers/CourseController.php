<?php defined('BASEPATH') OR exit('No direct script access allowed');
	class CourseController extends CI_Controller {
		...
		public function navigates(){
			$content = 'instructor/courses/privatecourse';
			$sum = $this->CourseModel->sumOfData($this->session->userdata('id_login'));
			$config['base_url'] = base_url().'CourseController/navigates/';
			$config['total_rows'] = $sum;
			$config['per_page'] = 3;
			$config['uri_segment'] = 3;
			$choice = $config["total_rows"] / $config["per_page"];
		...
		public function goToAddCourses(){
			$content = 'instructor/courses/addcourses';
			$data = array(
                'content' => $content,
                'username' => $this->session->userdata('username')
            );
		...
		public function addCourses(){
			...
			if ($result['result'] > 0) {
				...
				redirect('CourseController/goToAddCourses');
			} else {
				$this->session->set_flashdata('message','<div class="panel panel-default">
					<div class="media v-middle">
						<div class="media-left">
